...
def fetch_forecast(location: dict, *, forecast_days: int) -> tuple[str, dict]:
    ...
    try:
        ...
        raise UserFacingError(
            f"Open-Meteo päring ebaõnnestus asukoha {location['location_name']} jaoks: {exc}"
        ) from exc

    try:
        payload = response.json()
    except ValueError as exc:
        raise UserFacingError("Open-Meteo vastus ei olnud loetav JSON.") from exc
